...
export default function WeatherView() {
    ...
    const getWeatherAlertsAsync = async () => {
        ...
        try {
            const response = await fetch(
                "https://api.weatherapi.com/v1/alerts.json?key=0b50c9222bd8438ea0d232922252402&q=" +
    ...
    const getWeatherAsync = async () => {
        ...
        try {
            const response = await fetch(
                "https://api.weatherapi.com/v1/current.json?key=0b50c9222bd8438ea0d232922252402&q=" +
                    location.coords.latitude +
                    "," +
                    location.coords.longitude
            );
        ...
        <View style={styles.weatherContainer}>
            {weather === null ? (
                ...
                <View style={styles.weatherHeader}>
                    ...
                    <View style={styles.weatherContent}>
                        <WeatherIcon
                            iconPath={weather?.condition.icon}
                            altText={weather?.condition.text}
                        />
            ...
            <View style={styles.alertsContainer}>
                {alerts
                    ...
                    .filter(
                        (item, index, self) =>
                            index ===
                            self.findIndex((a) => a.headline === item.headline)
                    )
                    .map((item) => (
